...
Requires Node.js and the Xcode Command Line Tools (for `swiftc`/`codesign` — usually already present, as running `git` triggers their install).
...
- **Screen Recording** — `CGWindowListCopyWindowInfo` needs it to read *other* apps' window titles (meeting detection); without it, titles come back empty and no meeting is ever detected. Add the binary that runs the probe: for `node monitor.js` that's your terminal/Node; for autostart it's the process launchd runs
...
The real plist is gitignored (it contains an absolute Node path and HOME_SSID); use the `.example` as a template:
...
- **Does the Screen Recording grant survive a rebuild?** After granting once, run `./build.sh` and re-run `--dump-windows` *without* re-granting. If titles still come through, the `codesign --identifier` pinning worked; if not, re-granting after each rebuild is the accepted cost
...
5. **Virtual cameras trigger RED.** `isCameraInUse()` enumerates *all* CoreMediaIO devices, so an active OBS/Continuity/screen-capture pseudo-device reports RED even with the physical webcam off. If screen-sharing shows RED instead of YELLOW, filter the device list to physical cameras
