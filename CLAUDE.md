...
Upload `main.py`, `mdns.py`, and `dashboard.html` to the Pico 2 W using Thonny. `main.py` runs automatically on boot. A `secrets.py` must exist on the Pico (gitignored) with:
```python
SSID = "<wifi_name>"
...
### `main.py` — MicroPython on Raspberry Pi Pico 2 W
...
- Reachable at `http://onairsign.local` via a custom mDNS responder (`mdns.py`)
...
- `mdns.py` — minimal mDNS responder, makes the Pico reachable at `onairsign.local`
- `dashboard.html` — web control panel, served by Pico at `/` and also usable as a local file
